...
class TowerCashOutButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        game = tower_games.get(interaction.user.id)// ... existing code ...
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("This isn't your game!", ephemeral=True)
            return
        
        # ... (rest of the BlackjackView class)


# --- Tower Game Logic ---
TOWER_ROWS = 8
TOWER_COLS = 3
TOWER_MULTIPLIERS = [0, 1.4, 2.1, 3.15, 4.72, 7.08, 10.62, 15.94, 23.91] # Multipliers per row
...
class TowerGame:
    def __init__(self, bet_amount):
        ...
        self.game_over = False
        self.won = False
        # Generate a tower with one bomb per row
        self.tower = [random.randint(0, TOWER_COLS - 1) for _ in range(TOWER_ROWS)]
        self.player_path = []

    def select_tile(self, col):
        """Player selects a tile in the current row."""
        if self.game_over:
            return False, "Game is already over."

        is_bomb = (self.tower[self.current_row] == col)

        if is_bomb:
            self.game_over = True
            self.won = False
            self.player_path.append('💣')
            return False, "💥 You hit a bomb! Game over."
        else:
            self.player_path.append('💎')
            self.current_row += 1
            if self.current_row >= TOWER_ROWS:
                self.game_over = True
                self.won = True
                return True, "🎉 You reached the top of the tower!"
            return True, "✅ Safe! Move to the next row or cash out."

    def get_current_multiplier(self):
        return TOWER_MULTIPLIERS[self.current_row]

    def get_winnings(self):
        return self.bet_amount * self.get_current_multiplier()
...
class TowerView(View):
    ...
    def create_buttons(self):
        """Creates buttons for the current row."""
        # Clear previous buttons
        self.clear_items()
        
        # Add tile selection buttons
        for i in range(TOWER_COLS):
            button = Button(label=f"Tile {i+1}", style=discord.ButtonStyle.secondary, custom_id=f"tile_{i}")
            button.callback = self.tile_button_callback
            self.add_item(button)
        
        # Add cashout button
        cashout_button = Button(label="Cash Out", style=discord.ButtonStyle.success, custom_id="cashout")
        cashout_button.callback = self.cashout_button_callback
        self.add_item(cashout_button)

    async def update_view(self, interaction: discord.Interaction, message: str):
    ...
    def create_embed(self, message: str):
        ...
        color = discord.Color.green() if not self.game.game_over else (discord.Color.gold() if self.game.won else discord.Color.red())
        embed = discord.Embed(title="🗼 Tower Game", description=message, color=color)
        
        tower_display = ""
        for r in range(TOWER_ROWS -1, -1, -1):
            if r > self.game.current_row:
                tower_display += f"Row {r+1}: `[ ? ] [ ? ] [ ? ]`\n"
            elif r == self.game.current_row and not self.game.game_over:
                tower_display += f"Row {r+1}: `[ ? ] [ ? ] [ ? ]` ⬅️\n"
            else:
                # Show the result for completed rows
                tiles = ["💎"] * TOWER_COLS
                bomb_pos = self.game.tower[r]
                tiles[bomb_pos] = "💣"
                tower_display += f"Row {r+1}: `{' '.join(tiles)}`\n"


        embed.add_field(name="Tower", value=tower_display, inline=False)
        embed.add_field(name="Bet", value=f"${self.game.bet_amount:,}", inline=True)
        embed.add_field(name="Multiplier", value=f"x{self.game.get_current_multiplier():.2f}", inline=True)
        embed.add_field(name="Winnings", value=f"${self.game.get_winnings():,.2f}", inline=True)
        
        return embed

    async def tile_button_callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.user_id:
            await interaction.response.send_message("This isn't your game!", ephemeral=True)
            return

        tile_id = int(interaction.data["custom_id"].split("_")[1])
        is_safe, message = self.game.select_tile(tile_id)

        if self.game.game_over:
            if self.game.won: # Reached the top
                winnings = self.game.get_winnings()
                update_balance(self.user_id, winnings)
                message = f"🎉 You reached the top and won **${winnings:,.2f}!**"
            else: # Hit a bomb
                update_balance(self.user_id, -self.game.bet_amount)
                message = f"💥 You hit a bomb and lost **${self.game.bet_amount:,}**."
        
        await self.update_view(interaction, message)
    ...
    async def cashout_button_callback(self, interaction: discord.Interaction):
        ...
        if self.game.current_row == 0:
            await interaction.response.send_message("You can't cash out before starting!", ephemeral=True)
            return
        ...
        message = f"💸 You cashed out and won **${winnings:,.2f}!**"
        await self.update_view(interaction, message)
...
async def blackjack(interaction: discord.Interaction, bet: app_commands.Range[int, 1]):
    user_id = interaction.user.id

    if not can_afford(user_id, bet):
        await interaction.response.send_message("You don't have enough balance to place that bet.", ephemeral=True)
        return
...
# --- /tower command ---
@tree.command(name="tower", description="Play the Tower game")
// ... existing code ...
...
@tree.command(name="tower", description="Play the Tower game")
@app_commands.describe(bet="The amount you want to bet")
async def tower(interaction: discord.Interaction, bet: app_commands.Range[int, 1]):
    user_id = interaction.user.id
    ...
    game = TowerGame(bet_amount=bet)
    view = TowerView(game, user_id)
    
    embed = view.create_embed("Choose a tile from the bottom row to start.")
    await interaction.response.send_message(embed=embed, view=view)
...
async def on_ready():
    ...
    print("Commands ready: /ping, /userinfo, /balance, /blackjack, /tower, /wordle, /mines, /clearmines")
    print("🤖 Bot is now running 24/7 on Render!")
